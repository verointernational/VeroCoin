...
const string strMessageMagic = "VeroCoin Signed Message:\n";
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Oct 30, 2016. The VeroCoin was released, and hoping it is a great coin.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("VeroCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("VeroCoinMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("VeroCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("VeroCoin-miner");
    ...
    try { loop {
        ...
        printf("Running VeroCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
//            unsigned int nNonceFound;
    ...
    {
        printf("VeroCoinMiner terminated\n");
        throw;
    }
}
